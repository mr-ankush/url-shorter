...
public class twitter_short extends HttpServlet 
{
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        try 
        {
            ...
            String long_url="", short_url="", url_msg="", url_tag="",
            ...
            url_tag = request.getParameter("tag");
            url_tag = url_tag.toUpperCase();
            //out.println(url_tag+"<br>");
            long_url = request.getParameter("url");
            if(long_url!="")
            {
                Co = cod.encode(long_url);
                long_url = Co.getEncode();
                long_url = "http://twitter.com/share?text="+url_msg+"%0A&url="+long_url+"&hashtags="+url_tag;
            }
            else
            {
                long_url = "http://twitter.com/share?text="+url_msg+"%0A&url=none&hashtags="+url_tag;
            }
            //out.println(url+"<br>");
            ...
            {
                if(url_type.equals("Twitter Link"))
                {
    ...
    //                            "Don't refresh page otherwise shorted URL may be changed.");
                            ...
                            response.sendRedirect("http://www.urllive.in:8084/UrlLive/twitter/api/");
                        }
                        else
                        {
...
//                session.setAttribute("short_url",short_url);
        ...
        {
            Logger.getLogger(twitter_short.class.getName()).log(Level.SEVERE, null, e);
        }
    }
}